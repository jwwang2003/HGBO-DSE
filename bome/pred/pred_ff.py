...
# Flag for enabling Jumping Knowledge in HierNet (unused here)
jknFlag: bool = False


def ff_pred(data: Data) -> int:
    """
    Predict flip-flop (FF) usage from a PyG Data object using a pretrained HierNet.

    Args:
        data (Data): Graph data containing `x`, `edge_index`, and `'hls_attr'`.

    Returns:
        int: Rounded FF prediction.
    """
    # select device
    ...
    # post-process & return
    ff_value: float = out.view(-1).item()
    return round(ff_value)
