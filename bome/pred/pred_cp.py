...
def cp_pred(data: Data) -> float:
    """
    Predict critical path (CP) from a PyG Data object using a pretrained HierNet.

    Args:
        data (Data): Graph data containing `x`, `edge_index`, and `'hls_attr'` fields.

    Returns:
        float: Rounded CP prediction to three decimal places.
    """
    # select device
    ...
    # post-process
    cp_value: float = out.view(-1).item()
    return round(cp_value, 3)
